# Script para automatizar la actualización local y el lanzamiento a GitHub de YouTubeMp3Downloader
...
Write-Host "   ACTUALIZADOR AUTOMATICO DE YOUTUBEMP3" -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[INFO] 5. Creando etiqueta de version: $tagName..." -ForegroundColor Gray
git tag $tagName
if ($LASTEXITCODE -ne 0) { Write-Host "[ERROR] Error en git tag" -ForegroundColor Red; exit }

Write-Host "[INFO] 6. Subiendo etiqueta a GitHub (git push origin $tagName)..." -ForegroundColor Gray
...
if ($shouldWait) {
    ...
    $attempts = 0
    $maxAttempts = 24
    $apiUrl = "https://api.github.com/repos/francis02347/YouTubeMp3Downloader/releases/tags/$tagName"
    ...
    if ($released) {
        ...
        Write-Host " La version v$newVersionName ya esta disponible para descargar." -ForegroundColor Green
        Write-Host "==========================================================" -ForegroundColor Green
        Write-Host ""
        ...
        try {
            ...
            $notify.BalloonTipTitle = "YouTubeMp3Downloader - Version publicada!"
        ...
        Write-Host "https://github.com/francis02347/YouTubeMp3Downloader/actions" -ForegroundColor Cyan
        Write-Host ""
    }
} else {
    Write-Host ""
