...
Write-Host "[INFO] Iniciando compilacion local con Gradle..." -ForegroundColor Cyan
$gradleExe = "./gradle/bin/gradle.bat"
if (-not (Test-Path $gradleExe)) {
    $gradleExe = "./gradle/bin/gradle"
}
if (-not (Test-Path $gradleExe)) {
    $gradleExe = "gradle"
}
...
if ([string]::IsNullOrEmpty($githubToken) -or $githubToken -eq "TU_GITHUB_TOKEN_AQUI") {
    ...
    git commit -m "Version bump to $newVersionName"
    ...
    try {
        ...
        $uploadUrl = "https://uploads.github.com/repos/$owner/$repo/releases/$releaseId/assets?name=YouTubeMp3Downloader-$newVersionName.apk"
        ...
        $updateInfo = [PSCustomObject]@{
            ...
            apkUrl = $downloadUrl
            releaseNotes = "Actualizacion a la version $newVersionName"
        }
        ...
        git add update.json
        git commit -m "Update update.json to $newVersionName"
        git push origin main
        ...
        try {
            ...
            $notify.BalloonTipTitle = "YouTubeMp3Downloader - Publicada!"
